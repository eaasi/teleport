...
function getLocalConfig(): AppConfig {
	...
	if (process.env.VUE_APP_EDITION_TYPE !== undefined)
		config.EDITION_TYPE = process.env.VUE_APP_EDITION_TYPE;

	if (process.env.VUE_APP_TIME_OUT_DURATION !== undefined)
		config.TIME_OUT_DURATION = Number(process.env.VUE_APP_TIME_OUT_DURATION);

	return config;
}

function getRemoteConfig(): AppConfig {
	const config: AppConfig = (async () => {
